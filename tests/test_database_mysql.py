...
class TestDatabaseMySQL(TestBase):
    ...
    def _db_setup(self) -> bool:
        ...
        """
        args = ['resources/db_setup.sh', self._CREDS.get('password')]
        with subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE) as proc:
            _ = proc.communicate()
        # Invert the bit so exit code 0 is True, and visa versa.
        if proc.returncode ^ 1:
            dbi = DBInterface(connstr=self._CONNSTR)
            rtn = (pd.read_csv(os.path.join(self._DIR_DATA, 'data__guitars.csv'))
    ...
    def _db_teardown(self) -> bool:
        ...
        """
        args = ['resources/db_teardown.sh', self._CREDS.get('password')]
        with subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE) as proc:
            _ = proc.communicate()
        # Invert the bit so exit code 0 is True, and visa versa.
